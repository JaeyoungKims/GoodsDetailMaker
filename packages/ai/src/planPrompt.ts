...
const STAGE_GUIDE: Record<StoryStage, { goal: string; fallback: string }> = {
  ...
  PLAN: {
    goal: "이 상품을 고를 이유를 눈으로 납득시킨다. 무엇을 보여줄지는 품목에 맞춘다 — 먹는 것이면 속살·단면·조리한 모습, 입는 것이면 원단·바느질·핏, 기기라면 구조·기능·마감.",
    fallback:
      "정보가 없으면 사진에서 실제로 보이는 것만 묘사한다. 없는 기능이나 부품을 지어내지 않는다.",
  },
  OFFER: {
  ...
  PRODUCT_INFO: {
    goal: "구매 전 확인할 정보를 정리한다. 항목은 품목에 맞는 것만 고른다 — 농·수산물이면 중량·품종·산지·손질 상태·보관법·먹는 방법, 가공식품이면 용량·원재료·유통기한·보관법, 의류면 소재·사이즈·세탁법, 기기면 구성품·규격·사용법. 상품 정면 컷, 여백 넉넉하게.",
    fallback: "확인된 정보가 없으면 사진에서 보이는 것과 품목상 당연한 항목만 적고 수치는 비운다.",
  },
};
...
const ROLE_GUIDE: Record<(typeof SECTION_ROLES)[number], string> = {
  ...
  DETAIL: "상품의 실물감이 드러나는 근접 컷. 먹는 것이면 속살·단면, 만드는 것이면 마감·짜임.",
  ...
  PRODUCT_INFO: "정면 상품 컷, 카피 영역 크게 비움.",
};
...
6. 품목에 맞는 한국어를 쓴다. 농·수산물이나 식품에 "구성", "구성품", "사용 방법", "스펙" 같은 공산품 어휘를 쓰지 않는다. 고구마라면 "구성" 대신 중량·품종·손질 상태·보관법으로 적는다.

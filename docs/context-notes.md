...
타입체크·테스트 56개·웹 빌드까지는 통과했지만, **3단계 작업을 실제로 끝까지 돌려 본 검증은 아직이다.** OpenAI 키와 Postgres 가 붙은 환경에서 다음 두 가지를 확인해야 B1 이 끝난다.

1. 3단계만 골라 만든 작업이 기획 → 이미지 3장 → 미리보기 → ZIP·합본까지 성공
2. 단계 선택 이전에 만든 13장 작업이 그대로 열리고 내보내기까지 동작

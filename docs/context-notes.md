...
### 카피 프리셋·품목 어휘 실제 검증 (2026-09-04)

카드 프리셋에 `HERO · GAP · PRODUCT_INFO` 3단계, 옵션 없음으로 작업 `a3332ebc` 를 실제로 돌렸다.

- 본문 3장이 흰 카드 판 위에 검정 글자로 합성됐다. 판은 글자가 차지한 만큼만 그려진다.
- 옵션이 없어도 대표 썸네일 1장이 자동으로 만들어졌다.
- PRODUCT_INFO 카피가 `원산지: 해남 / 손질 상태: 세척 / 권장 섭취: 바로 구워 먹기` 로 나왔다. 고치기 전에는 `구성과 간단한 사용 안내 / 껍질 세척된 고구마 구성` 이었다. 품목에 맞는 어휘로 바뀐 것을 확인했다.

작업 하나에 이미지 호출은 본문 3 + 썸네일 1 = 4회였다.

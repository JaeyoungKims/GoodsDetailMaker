...
| 마켓 썸네일   | 옵션별 정사각 썸네일, 격자 합성 메인, AI 배치 메인, ZIP 포함                       | **옵션 2개 실제 생성 성공** |
| 문서          | setup.md, architecture.md, CLAUDE.md, checklist.md, context-notes.md               | -                           |
...
- 2026-09-04: 내보내기 확인. 옵션 작업 ZIP 에 본문 1장(1024×1536)과 썸네일 4장(1000×1000)이 담기고, 세로 합본도 정상. 옵션 없는 13장 작업 ZIP 은 13개 파일에 썸네일이 섞이지 않는다.
- 2026-09-04: B2 썸네일. 옵션 2개 작업에서 옵션 썸네일 2장(1024×1024) → 1000×1000 축소, 격자 메인 합성, AI 배치 메인까지 실제 성공. 옵션 없는 기존 작업은 thumbnails 가 빈 배열로 그대로 동작.
- 2026-09-04: B1 단계 선택. 3단계(HERO·GAP·PRODUCT_INFO)만 골라 기획 → 이미지 3장 생성까지 실제 성공. 기존 13장 작업도 그대로 열린다.
...
B1(단계 선택)·B2(마켓 썸네일)는 완료해 위 표로 옮겼다.

### B1. 단계 1~13 선택 (구현·서버 검증 완료, 브라우저 내보내기 확인만 남음)
